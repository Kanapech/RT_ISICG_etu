...
namespace RT_ISICG
{
	struct AABB
	{
	  public:
		...
		{ 
			_min.x = glm::min( _min.x, p_point.x );
			_min.y = glm::min( _min.y, p_point.y );
			_min.z = glm::min( _min.z, p_point.z );

			_max.x = glm::max( _max.x, p_point.x );
			_max.y = glm::max( _max.y, p_point.y );
			_max.z = glm::max( _max.z, p_point.z );
		}

		// Extends the AABB with another AABB
		inline void extend( const AABB & p_aabb )
		{
			_min.x = glm::min( _min.x, p_aabb._min.x );
			_min.y = glm::min( _min.y, p_aabb._min.y );
			_min.z = glm::min( _min.z, p_aabb._min.z );

			_max.x = glm::max( _max.x, p_aabb._max.x );
			_max.y = glm::max( _max.y, p_aabb._max.y );
			_max.z = glm::max( _max.z, p_aabb._max.z );
		}
